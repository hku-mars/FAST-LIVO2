...
class Preprocess {
 public:
  ...
  int lidar_type, point_filter_num, N_SCANS, imag_type;
 ...
 private:
  ...
  bool edge_jump_judge(const PointCloudXYZI& pl, vector<orgtype>& types, uint i,
                       Surround nor_dir);
  void StandHandler(const sensor_msgs::PointCloud2::ConstPtr& msg);
  int group_size;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
    livox_ros::Point,
    (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)(
        float, time_interval, time_interval)(float, offset_time, offset_time)(
        std::uint8_t, return_value, return_value)(std::uint8_t, ring, ring))
