...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
    ...
    nh.param<bool>("common/dynamic_slam", dynamic_slam, false);
    ...
    if (dynamic_slam)
    {
        if(is_compressed_image)
        {
            ...
            com_sync = std::make_unique<message_filters::Synchronizer<CompressedSyncPolicy>>(
                CompressedSyncPolicy(10), img_sub, mask_sub);
        
            com_sync->registerCallback(boost::bind(&LIVMapper::compressed_mask_cbk, this, _1, _2));
        }
        else
        {
            ...
            img_sync = std::make_unique<message_filters::Synchronizer<ImageSyncPolicy>>(
                ImageSyncPolicy(10), img_sub, mask_sub);
        
            img_sync->registerCallback(boost::bind(&LIVMapper::img_mask_cbk, this, _1, _2));
        }
...
void LIVMapper::img_mask_cbk(const sensor_msgs::ImageConstPtr &img_msg, const sensor_msgs::ImageConstPtr &mask_msg)
{
    // if (!img_en)
    //     return;
