...
void Preprocess::urbanloco_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();

    pcl::PointCloud<urbanloco_ros::Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.points.size();
    std::cout << "[UrbanlocoHandler] plsize: " << plsize << std::endl;
    if (plsize == 0)
        return;
    pl_surf.reserve(plsize);

    bool is_first[32];
    double yaw_fp[32] = {0};     // yaw of first scan point
    double omega_l = 3.61;       // scan angular velocity
    float yaw_last[32] = {0.0};  // yaw of last scan point
    float time_last[32] = {0.0}; // last offset time
    ...
    double yaw_end = yaw_first;
    int layer_first = pl_orig.points[0].ring;
    std::cout << "layer_first: " << layer_first << std::endl;
    for (uint i = plsize - 1; i > 0; i--)
    {
        if (pl_orig.points[i].ring == layer_first)
        {
    ...
    std::cout << "yaw_end: " << yaw_end << std::endl;
    for (int i = 0; i < plsize; i++)
    {
        PointType added_pt;
    ...
    std::cout << "[UrbanLocoHandler] end. " << std::endl;
    // pub_func(pl_surf, pub_full, msg->header.stamp);
    // pub_func(pl_surf, pub_surf, msg->header.stamp);
    // pub_func(pl_surf, pub_corn, msg->header.stamp);
}
